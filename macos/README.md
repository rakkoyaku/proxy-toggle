...
メニューバーアイコンの右クリックメニュー →「ログイン時に起動」で切り替えられます。

- **macOS 13 以降**: `SMAppService` でアプリ自身を登録します。システム設定 → 一般 → ログイン項目 に表示され、そこからも切り替えられます。
  macOS が承認待ちにした場合はメニュー項目が「システム設定で承認が必要」に変わり、設定画面へ誘導します。
- **macOS 12**: `SMAppService` が無いため、`~/Library/LaunchAgents/local.proxytoggle.agent.plist` を作成します。
  13 以降でも登録が拒否された場合はこちらにフォールバックします。

plist は書き出すだけで `launchctl bootstrap` はしません。目的は「次回ログイン時に起動すること」であり、
その場でロードすると起動中のアプリと二重に立ち上がってしまうためです。

`./uninstall.sh` は LaunchAgent も削除します（`SMAppService` の登録はアプリ本体の削除に伴い OS 側で失効します）。
